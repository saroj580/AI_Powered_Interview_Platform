...
interface Props {
  onAnalysis: (result: ResumeAnalysis) => void;
}

export function ResumeUploadZone({ onAnalysis }: Props) {
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      onAnalysis(data.analysis as ResumeAnalysis);
    } catch {
          ...
          <div className="flex items-center justify-between p-4 rounded-xl bg-primary/5 border border-primary/20">
            ...
            <div className="flex items-center gap-2">
              <Button size="sm" onClick={handleAnalyze} disabled={analyzing} className="bg-gradient-primary text-white border-0 hover:opacity-90 gap-2">
                {analyzing ? <><Loader2 className="h-3.5 w-3.5 animate-spin" />Analyzing…</> : "Analyze with AI"}
              </Button>
              <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { setFile(null); setError(null); }}>
                <X className="h-4 w-4" />
              </Button>
            </div>
          </div>
